...
async def test_valid_token_returns_user():
    ...
    with (
        patch("app.auth.jwt.decode", return_value={"sub": "test@example.com"}),
...
async def test_token_user_not_found_returns_none():
    ...
    with (
        patch("app.auth.jwt.decode", return_value={"sub": "ghost@example.com"}),
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    not _HAS_FULL_APP, reason="Full app dependencies not installed (e.g. fitz)"
)
