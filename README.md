...
| **Eval corpus + comprehension scoring** — 1,891 automated tests + a separate QA corpus that scores end-to-end conversation understanding | [`tests/simulation/corpus/`](tests/simulation/corpus/) |
...
- **1,891 automated tests** (`pytest --collect-only`), ~55% coverage,
  currently ~9 failing locally on Python 3.14 (bcrypt compat); CI uses
  3.10 and is green.
...
tests/
  ...
  test_*.py             Unit + integration tests (~1,891 total)
